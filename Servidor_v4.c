...
void *AtenderCliente (void *socket){
	...
	while (terminar == 0){
		...
		if (codigo==1){ //Consulta 1
			...
			write(sock_conn,respuesta,strlen(respuesta));
				...
				if (strcmp(password, contrasena)==0)
				{
					...
					pthread_mutex_unlock(&mutex);
					if(respon == 0){
						printf("Usuario aï¿½adido\n");
					}
					else
					   printf("Usuario no aï¿½adido\n");
		...
		else if (codigo == 7){//Funciï¿½n para la invitaciï¿½n a una partida
			char jugador[20];
			p = strtok(NULL,"/");
			strcpy(nombre, p);
			p = strtok(NULL,"/");
		...
		if ((codigo == 4) || (codigo == 0)){ // Para 0 y 4, quita y aï¿½ade los jugadores a la lista de conectados respectivamente (NOTIFICACION)
...
int consulta3(char respuesta[512], char nombre[20]){ //Nos da la contraseï¿½a del usuario seleccionado. (0 al mostrarla y -1 si no existe el jugador)
	...
	{
		printf ("La contraseï¿½a de ese usuario es: %s\n", row[0] );
...
int Registro(char respuesta[512], char nombre[20], char contrasena[20]){ //Funcion que registra a un jugador nuevo. (0 al aï¿½adirlo y -1 si ha ocurrido algun error)
...
int Pon (ListaConectados *milista, char nombre[20], int socket){
	//Aï¿½ade el nuevo conectado y informa si estï¿½ llena o no 
	if (milista->num== 100)
		return -1;
	else{
...
int CrearPartida(ListaPartidas *mispartidas, char nombre[20], int numerojugadores){//Aï¿½ade una partida a la lista de partidas
	...
	if(mispartidas->num== 100){
		...
		int devolver = (mispartidas->num-1);
		return devolver;
	}
}

int PonerEnPartida(ListaPartidas *mispartidas, int Id, char jugador[20]){//Aï¿½ade un jugador a la partida seleccionada
...
int main(int argc, char *argv[]){
	...
	serv_adr.sin_port = htons(9101); //AQUI SE PONE EL PUERTO
	...
	conn = mysql_real_connect (conn, "localhost", "root", "mysql", "MEscobar_DB",0, NULL, 0);
	if (conn==NULL)
	{
   ...
   //Variables para sockets:
	...
	for(;;)
	{
		...
		printf("Se ha recibido la conexion\n");
